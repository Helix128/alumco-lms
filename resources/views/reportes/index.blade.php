...
<body class="bg-Alumco-cream font-sans leading-normal tracking-normal text-Alumco-gray h-screen flex overflow-hidden">
    ...
    <aside class="w-20 bg-Alumco-blue flex flex-col items-center py-6 gap-8 z-10 shrink-0">
        ...
        <form method="POST" action="{{ route('logout') }}" class="mt-auto">
            @csrf
            <button type="submit" class="text-white hover:text-Alumco-cyan cursor-pointer bg-transparent border-none focus-ring p-1 rounded-Alumco" title="Cerrar sesión">
                <!-- Icono de Logout -->
            ...
            <form action="{{ route('reportes.index') }}" method="GET" class="bg-white border border-Alumco-gray/15 rounded-Alumco p-4 md:p-5">
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
                    ...
                    <div class="lg:col-span-12 flex flex-wrap gap-2 pt-1">
                        ...
                        <a href="{{ route('reportes.index') }}" class="bg-gray-200 hover:bg-gray-300 text-Alumco-gray font-bold py-2.5 px-7 rounded-Alumco shadow transition-colors flex items-center">Limpiar todo</a>
                    </div>
                </div>
            </form>
        </div>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            if (pickerRoot) {
                ...
                const updatePickerView = () => {
                    ...
                    selected.slice(0, 8).forEach((option) => {
                        ...
                        chip.setAttribute('aria-label', 'Quitar curso ' + option.dataset.courseLabel);
                ...
                document.addEventListener('click', function (event) {
                    const clickedInside = pickerRoot.contains(event.target);
                    if (!clickedInside && !panel.classList.contains('hidden')) {
                        closePanel();
                    }
                });

                options.forEach((option) => {
                    option.addEventListener('change', updatePickerView);
                });
            ...
            if (ageRoot) {
                ...
                const updateRangeFill = () => {
                    ...
                    const left = ((min - minRange) / totalRange) * 100;
                    const right = ((max - minRange) / totalRange) * 100;
